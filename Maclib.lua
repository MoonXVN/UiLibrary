...
function MacLib:Window(Settings)
    ...
    local agasgasgs = game:GetService("UserInputService")
    local gagsagsa = game:GetService("TweenService")
    
    local function avav(topbarobject, object)
    local asfasfa = nil
    local agasgsaga = nil
    local ahsahsaha = nil
    local gasgasgsg = nil
    ...
    local Delta = input.Position - ahsahsaha
    local pos =
    UDim2.new(
    gasgasgsg.X.Scale,
    gasgasgsg.X.Offset + Delta.X,
    gasgasgsg.Y.Scale,
    gasgasgsg.Y.Offset + Delta.Y
    )
    local Tween = gagsagsa:Create(object, TweenInfo.new(0.2), {Position = pos})
    Tween:Play()
    end
    ...
    asfasfa = true
    ahsahsaha = input.Position
    gasgasgsg = object.Position
    ...
    agasgasgs.InputChanged:Connect(
    function(input)
    if input == agasgsaga and asfasfa then
    Update(input)
    end
    end
    )
	...
	sidebarGroupUIPadding.PaddingTop = UDim.new(0, 50)
	...
	moveIcon.Size = UDim2.fromOffset(50, 50)
    ...
    interact.MouseButton1Click:Connect(function()
        setclipboard('https://discord.com/1234')
					...
					local function Toggle(optionName, State)
						...
						if State then
							if DropdownFunctions.Settings.Multi then
								...
								DropdownFunctions.Settings.Callback(Selected[1])
							end
